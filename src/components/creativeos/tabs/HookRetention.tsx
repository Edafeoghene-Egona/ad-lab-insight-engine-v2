...
import { DropoffMap } from "../charts/DropoffMap";
...
export function HookRetention({
  ...
  if (withQ.length === 0) {
    return (
      <EmptyState
        ...
        hint="Video quartile metrics weren’t returned for these creatives (common for Demand Gen). Other metrics are in the Creative Testing Lab."
      />
    );
  }

  const avg = (pick: (q: NonNullable<Creative["quartiles"]>) => number) =>
    withQ.reduce((s, c) => s + pick(c.quartiles!), 0) / withQ.length;
  ...
  const noteQ = noQ > 0 ? `${noQ} creative(s) without quartile data (Demand Gen) are excluded.` : undefined;

  if (sub === "Quartile Funnel") {
    return (
      <div className="cos-reveal flex flex-col gap-6">
        ...
        {noteQ && <p className="text-xs text-slate-400">{noteQ}</p>}
      </div>
    );
  }

  if (sub === "Retention Curves") {
    return (
      <div className="cos-reveal flex flex-col gap-6">
        <GlassPanel className="p-6">
    ...
    return (
      <div className="cos-reveal flex flex-col gap-4">
        <p className="text-sm text-slate-500">
          Where each creative loses the viewer. <span className="text-slate-700 font-semibold">↳ Fix the second that kills retention.</span>
        </p>
        <GlassPanel className="p-6">
          <DropoffMap creatives={withQ} onOpen={onOpenCreative} />
        </GlassPanel>
      </div>
    );
  }

  // Hook Rate Ranking (default)
  return (
    <div className="cos-reveal flex flex-col gap-6">
      <GlassPanel className="p-6">
